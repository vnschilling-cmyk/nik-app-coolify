...
export function calculateGrade(percentage: number): GradeInfo {
    ...
    if (percentage >= 65) return { grade: 3, label: "Befriedigend", color: "text-amber-500" };
    if (percentage >= 50) return { grade: 4, label: "Ausreichend", color: "text-amber-400" };
...
export function getGradeColor(grade: number): string {
    switch (grade) {
        ...
        case 2: return "bg-emerald-400/10 text-emerald-400 border-emerald-400/20";
        case 3: return "bg-amber-500/10 text-amber-500 border-amber-500/20";
        case 4: return "bg-amber-400/10 text-amber-400 border-amber-400/20";
